...
export default function ScalesPage({ params }: { params: Promise<{ frameworkId: string }> }) {
  ...
  const addLevel = () => setForm((f: any) => ({ ...f, levels: [...f.levels, { value: f.levels.length.toString(), label: "", label_ar: "", description: "", description_ar: "", color: "", sort_order: f.levels.length, min_threshold: "", max_threshold: "" }] }));
  ...
  const handleSave = () => {
    ...
    const payload: any = { ...form, min_value: form.min_value ? parseFloat(form.min_value) : null, max_value: form.max_value ? parseFloat(form.max_value) : null, step: form.step ? parseFloat(form.step) : null,
      levels: form.levels.map((l: any, i: number) => ({ ...l, value: parseFloat(l.value), sort_order: i,
        min_threshold: l.min_threshold !== "" && l.min_threshold != null ? parseFloat(l.min_threshold) : null,
        max_threshold: l.max_threshold !== "" && l.max_threshold != null ? parseFloat(l.max_threshold) : null,
      })),
    };
      ...
      <ImportPreviewModal open={!!importPreview} preview={importPreview} loading={importing} itemLabel="scales" nameKey="name"
        ...
        onConfirm={async () => {
          ...
          try {
            ...
            toast(`Imported ${d.imported_scales} scales (${d.skipped_duplicates} skipped)`, "success"); queryClient.invalidateQueries({ queryKey: ["scales"] });
